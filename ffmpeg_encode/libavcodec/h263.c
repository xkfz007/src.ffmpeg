...
static void aspect_to_info(MpegEncContext * s, AVRational aspect){
    ...
    if(aspect.num==0) aspect.num = aspect.den = 1;
...
void h263_encode_picture_header(MpegEncContext * s, int picture_number)
{
    ...
    if(s->h263_plus){
        for(i=0; i<2; i++){
            int div, error;
            div= (s->avctx->time_base.num*((int64_t)1800000) + ((int64_t)500)*s->avctx->time_base.den) / ((((int64_t)1000)+i)*s->avctx->time_base.den);
            div= clip(1, div, 127);
            error= ABS(s->avctx->time_base.num*((int64_t)1800000) - (((int64_t)1000)+i)*s->avctx->time_base.den*div);
            if(error < best_error){
                best_error= error;
                best_divisor= div;
                best_clock_code= i;

...
static int find_frame_rate_index(MpegEncContext *s){
    ...
    for(i=1;i<14;i++) {
        int64_t n0= ((int64_t)1001)/frame_rate_tab[i].den*frame_rate_tab[i].num*s->avctx->time_base.num;
        int64_t n1= ((int64_t)1001)*s->avctx->time_base.den;
     ...
     //that behave like P frames
        ...
        //mpeg2 aspect
            if(s->aspect_ratio_info > 1){
				...
				{
					AVRational ttmp = {s->width, s->height};
                    s->avctx->sample_aspect_ratio= av_div_q(mpeg2_aspect[s->aspect_ratio_info], ttmp );
                }
				else
				{
					AVRational ttmp = {s1->pan_scan.width, s1->pan_scan.height};
                    s->avctx->sample_aspect_ratio= av_div_q(mpeg2_aspect[s->aspect_ratio_info], ttmp );
                }
            }else{
...
enum PixelFormat pix_fmts[]= {PIX_FMT_YUV420P, -1};

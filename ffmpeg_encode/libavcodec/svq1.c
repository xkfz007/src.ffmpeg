                             ...
                             uint8_t *buf, int buf_size)
{
  ...
  for (i=0; i < 3; i++) {
	...
	{
      svq1_pmv_t *pmv=malloc(width/8+3*sizeof(svq1_pmv_t));
...
//        s->m.unrestricted_mv= 1;
        ...
        for (y = 0; y < block_height; y++) {
            uint8_t *src=malloc(stride*16*sizeof(uint8_t));
    ...
    for (y = 0; y < block_height; y++) {
        ...
        for (x = 0; x < block_width; x++) {
            ...
            uint8_t *temp=malloc(16*stride*sizeof(uint8_t));
...
static enum PixelFormat pix_fmt[]={PIX_FMT_YUV410P, -1};
...
AVCodec svq1_decoder = {
    ...
    ff_mpeg_flush,
	NULL,
    pix_fmt, //(enum PixelFormat[]){PIX_FMT_YUV410P, -1},
};
